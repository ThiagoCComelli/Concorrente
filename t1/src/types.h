...
typedef struct cookerOrdersReady_s {
	order_t* data;
	int actualSize;
} cookerOrdersReady_t;

typedef struct cookerOrders_s {
	order_t* data;
	int actualSize;
} cookerOrders_t;
...
TicketCaller* init_ticket_caller() {
	...
	pc->lastCheckPwd = 0;
	pc->lastCheckReady = 0;
	pc->clientsWaitingOrder = 0;
	return pc;
}

// CLIENTE PEGA UM TICKET
...
// PROXIMO NUMERO A SER PEGADO PELO ATENDENTE
